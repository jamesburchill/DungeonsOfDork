...
class Player:
    ...
    def __init__(self, loc_list, obj_list, npc_list, meta=None, meta_path=None, mutator=None, player_class=None):
        self.locs = loc_list
    ...
    def move(self, direction, allow_bonus=True):
        ...
        if allow_bonus and self.map_boost_active and self.perks["extra_move_on_map"]:
            self.map_boost_active = False
            bonus = input("Map insight grants a bonus move (N/S/E/W or skip) > ").strip().upper()
    ...
    def drop_object(self, selection=None):
        ...
        if selection:
            ...
            drop = input("Which object do you wish to drop? Type number or name > ").strip()
        ...
        confirm = input(f"Drop {self.obj_by_id[target_item_id].Name}? Y/N > ").strip().upper()
    ...
    def resolve_attack(self):
        ...
        if weakness and self.has_item(weakness):
            self.say(f"You exploit {npc.Name}'s weakness using {self.obj_by_id[weakness].Name}.", "green", log=True)
            return self.defeat_npc(npc)

        dmg = 12 + self.mutator.get("enemy_damage_bonus", 0)
        self.health -= dmg
        self.say(f"You strike {npc.Name}, but it shrugs it off. You lose {dmg} health.", "red", log=True)
        return True
...
if __name__ == "__main__":
    ...
    npc_fname = data_dir / "npcs.csv"
    gen_fname = data_dir / "genlocs.csv"
    obj_fname = data_dir / "objects.csv"
    loc_fname = data_dir / "locations.csv"
    meta_path = data_dir / "meta.json"

    meta = load_meta(meta_path)
    mutator = choose_mutator()

    gens = genlocs_from_file(gen_fname)
    npcs = npcs_from_file(npc_fname)
    objs = objects_from_file(obj_fname)
    locs = locations_from_file(loc_fname, gens)
    prepare_world(locs, objs, npcs)

    player_class = choose_class(meta)
    player = Player(locs, objs, npcs, meta=meta, meta_path=meta_path, mutator=mutator, player_class=player_class)
